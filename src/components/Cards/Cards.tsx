...
//   title: string;
// };
// export const Cards: React.FC<Props> = ({ filteredProducts, title}) => {
//   const contentRef = useRef<HTMLDivElement>(null);
//   const listRef = useRef<HTMLDivElement>(null);
//   const [visibleWidth, setVisibleWidth] = useState<number>(0);
//   const [currentIndex, setCurrentIndex] = useState(0);
...
export const Cards: React.FC<Props> = ({ filteredProducts, title }) => {
  ...
  const maxIndex = useMemo(() => {
    if (fullWidthOneCard === 0) return 0;

    const totalSteps = Math.max(
      Math.ceil(filteredProducts.length - visibleWidth / fullWidthOneCard),
      0
    );

    return totalSteps;
  }, [filteredProducts.length, visibleWidth, fullWidthOneCard]);
  ...
  useEffect(() => {
    ...
    const stepSize = fullWidthOneCard; // Шаг = 1 карточка
    const offset = stepSize * currentIndex;
