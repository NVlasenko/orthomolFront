...
export const ShopItems: React.FC<{ filters: Filters }> = ({ filters }) => {
  ...
  useEffect(() => {
    ...
    updatedProducts = sortProducts(updatedProducts, sortCriteria);
    setFilteredProducts(updatedProducts);
    setCurrentIndex(0); // Сбросить индекс, если обновились продукты
  }, [filters, sortCriteria, products]);
  ...
  return (
    <div className="shopItems">
      ...
      <motion.div className="shopItems__carousel" {...handlers}>
        <div
          className="shopItems__carousel-wrapper"
          style={{ transform: `translateY(calc(-${currentIndex * 100}%))`, transition: "transform 0.5s ease-in-out" }}
        >
          {filteredProducts.length > 0 ? (
            filteredProducts.map((product, index) => (
              <motion.div
                key={product.id}
                className={`shopItems__carousel-item ${index === currentIndex ? "active" : ""}`}
              >
                <Card product={product} />
              </motion.div>
            ))
